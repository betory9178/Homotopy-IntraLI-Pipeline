...
atlasflag='AIC';
subid=ID.StID;
statspath='/data/stalxy/ArticleJResults/HCP/Results/CCA_AICHA_NGR/24_rest/';
% shp='/data/stalxy/ArticleJResults/HCP/Results/CCA_AICHA_NGR/plot.sh';
shp='/data/stalxy/ArticleJResults/HCP/Results/CCA_AICHA_NGR/plot_24_rest.sh';
...
%% type 0
% [Brain_COEF,Behaviour_COEF,CCA_T_r,Brain_ComSR,Behaviour_ComSR,CCA_T_stats,Brain_LD,Behaviour_LD,Brain_CrsLD,Behaviour_CrsLD] = ZCX_canoncorr([ho_FC1(~absences,:),LI_intra_abs1(~absences,:),ho_FC2(~absences,:),LI_intra_abs2(~absences,:)],behaviours_ageadj(~absences,:));
% brain_CCA_num=sum(CCA_T_stats.pChisq<0.05);
% [CCA_Bload_Order,CCA_B_Ind]=sort(Behaviour_LD,'descend');
% beh_order=titles(CCA_B_Ind(:,1:brain_CCA_num));
% for i=1:brain_CCA_num
% PlotCorr([statspath,'/all/'],['AllxBehav_Rest1_CCA_',num2str(i)],Brain_ComSR(:,i),Behaviour_ComSR(:,i));
% PlotCorr([statspath,'/all/'],['Brain_Homo_Rest1xTask_LOAD_',num2str(i)],Brain_LD(1:length(nid),i),Brain_LD(2*length(nid)+1:3*length(nid),i));
% PlotCorr([statspath,'/all/'],['Brain_LIabs_Rest1xTask_LOAD_',num2str(i)],Brain_LD(length(nid)+1:2*length(nid),i),Brain_LD(3*length(nid)+1:4*length(nid),i));
% PlotCorr([statspath,'/all/'],['Brain_HomoxLIabs_Rest1_LOAD_',num2str(i)],Brain_LD(1:length(nid),i),Brain_LD(length(nid)+1:2*length(nid),i));
% PlotCorr([statspath,'/all/'],['Brain_HomoxLIabs_Task_LOAD_',num2str(i)],Brain_LD(2*length(nid)+1:3*length(nid),i),Brain_LD(3*length(nid)+1:4*length(nid),i));
% 
% homo_all_ld1=zeros(192,1);
% homo_all_ld2=zeros(192,1);
% homo_all_ld1(nid)=Brain_LD(1:length(nid),i);
% homo_all_ld2(nid)=Brain_LD(2*length(nid)+1:3*length(nid),i);
% 
% liabs_all_ld1=zeros(192,1);
% liabs_all_ld2=zeros(192,1);
% liabs_all_ld1(nid)=Brain_LD(length(nid)+1:2*length(nid),i);
% liabs_all_ld2(nid)=Brain_LD(3*length(nid)+1:4*length(nid),i);
% 
% SaveAsAtlasMZ3_Plot(homo_all_ld1,[statspath,'/all/'],['all_Homo_Rest1_LOAD_' num2str(i)],[-0.15,0.15],shp);
% SaveAsAtlasMZ3_Plot(homo_all_ld2,[statspath,'/all/'],['all_Homo_Task_LOAD_' num2str(i)],[-0.15,0.15],shp);
% SaveAsAtlasMZ3_Plot(liabs_all_ld1,[statspath,'/all/'],['all_LIabs_Rest1_LOAD_' num2str(i)],[-0.15,0.15],shp);
% SaveAsAtlasMZ3_Plot(liabs_all_ld2,[statspath,'/all/'],['all_LIabs_Task_LOAD_' num2str(i)],[-0.15,0.15],shp);
% 
% end
...
% [CCA_Bload_Order2,CCA_B_Ind2]=sort(Behaviour_LD2,'descend');
% beh_order2=titles(CCA_B_Ind2(:,1:brain_CCA_num2));

% xlsname='/data/stalxy/ArticleJResults/HCP/Results/CCA_AICHA_NGR/CCA_BehavLoading.xls';
% messageflag = xlswrite(xlsname, CCA_Bload_Order1(:,1:brain_CCA_num1),'Sheet1','A1')
% messageflag = xlswrite(xlsname, beh_order1{1},'Sheet2','A1')
% messageflag = xlswrite(xlsname, CCA_Bload_Order2(:,1:brain_CCA_num2),'Sheet3','A1')      
% messageflag = xlswrite(xlsname, beh_order2{1},'Sheet4','A1')
...
%         PlotCorr([statspath,'/Combined/'],['CombxBehav_Task_CCA_',num2str(j)],Brain_ComSR2(:,j),Behaviour_ComSR2(:,j));
...
% 
%         PlotCorr([statspath,'/Combined/'],['Brain_Homo_Rest1xTask_LOAD_',num2str(i),'x',num2str(j)],Brain_LD1(1:length(nid),i),Brain_LD2(1:length(nid),j));
%         PlotCorr([statspath,'/Combined/'],['Brain_LIabs_Rest1xTask_LOAD_',num2str(i),'x',num2str(j)],Brain_LD1(length(nid)+1:2*length(nid),i),Brain_LD2(length(nid)+1:2*length(nid),j));
%       
...
%         SaveAsAtlasMZ3_Plot(liabs_comb_co2,[statspath,'/Combined/'],['Comb_LIabs_Task_COEF_' num2str(j)],[-0.2,0.2],shp);
        ...
        SaveAsAtlasMZ3_Plot(homo_comb_ld1,[statspath,'/Combined/'],['Comb_Homo_Rest1_LOAD_' num2str(i)],[-0.15,0.15],shp);
%         SaveAsAtlasMZ3_Plot(homo_comb_ld2,[statspath,'/Combined/'],['Comb_Homo_Task_LOAD_' num2str(j)],[-0.15,0.15],shp);
        SaveAsAtlasMZ3_Plot(liabs_comb_ld1,[statspath,'/Combined/'],['Comb_LIabs_Rest1_LOAD_' num2str(i)],[-0.15,0.15],shp);
%         SaveAsAtlasMZ3_Plot(liabs_comb_ld2,[statspath,'/Combined/'],['Comb_LIabs_Task_LOAD_' num2str(j)],[-0.15,0.15],shp);
...
% 
% [Brain_COEF1_wCOV,Behaviour_COEF1_wCOV,CCA_T_r1_wCOV,Brain_ComSR1_wCOV,Behaviour_ComSR1_wCOV,CCA_T_stats1_wCOV,Brain_LD1_wCOV,Behaviour_LD1_wCOV,Brain_CrsLD1_wCOV,Behaviour_CrsLD1_wCOV] = ZCX_canoncorr([ho_FC1(~absences,:),LI_intra_abs1(~absences,:),age(~absences,1),gennum(~absences,1)],behaviours_ageadj(~absences,:));
% [Brain_COEF2_wCOV,Behaviour_COEF2_wCOV,CCA_T_r2_wCOV,Brain_ComSR2_wCOV,Behaviour_ComSR2_wCOV,CCA_T_stats2_wCOV,Brain_LD2_wCOV,Behaviour_LD2_wCOV,Brain_CrsLD2_wCOV,Behaviour_CrsLD2_wCOV] = ZCX_canoncorr([ho_FC2(~absences,:),LI_intra_abs2(~absences,:),age(~absences,1),gennum(~absences,1)],behaviours_ageadj(~absences,:));
% 
% brain_CCAwc_num1=sum(CCA_T_stats1_wCOV.pChisq<0.05);
% brain_CCAwc_num2=sum(CCA_T_stats2_wCOV.pChisq<0.05);
% 
% [CCAwc_Bload_Order1,CCAwc_B_Ind1]=sort(Behaviour_LD1_wCOV,'descend');
% beh_order1_wCOV=titles(CCAwc_B_Ind1(:,1:brain_CCAwc_num1));
% 
% [CCAwc_Bload_Order2,CCAwc_B_Ind2]=sort(Behaviour_LD2_wCOV,'descend');
% beh_order2_wCOV=titles(CCAwc_B_Ind2(:,1:brain_CCAwc_num2));
% 
% for i=1:brain_CCAwc_num1
%     for j=1:brain_CCAwc_num2
%         
%         PlotCorr([statspath,'/Combined_wCOV/'],['CombxBehav_Rest1_CCA_',num2str(i)],Brain_ComSR1_wCOV(:,i),Behaviour_ComSR1_wCOV(:,i));
%         PlotCorr([statspath,'/Combined_wCOV/'],['CombxBehav_Task_CCA_',num2str(j)],Brain_ComSR2_wCOV(:,j),Behaviour_ComSR2_wCOV(:,j));
%                
%         PlotCorr([statspath,'/Combined_wCOV/'],['Behav_Comb_Rest1xTask_LOAD_',num2str(i),'x',num2str(j)],Behaviour_LD1_wCOV(:,i),Behaviour_LD2_wCOV(:,j));
%         PlotCorr([statspath,'/Combined_wCOV/'],['Brain_Comb_Rest1xTask_LOAD_',num2str(i),'x',num2str(j)],Brain_LD1_wCOV(:,i),Brain_LD2_wCOV(:,j));
% 
%         PlotCorr([statspath,'/Combined_wCOV/'],['Brain_Homo_Rest1xTask_LOAD_',num2str(i),'x',num2str(j)],Brain_LD1_wCOV(1:length(nid),i),Brain_LD2_wCOV(1:length(nid),j));
%         PlotCorr([statspath,'/Combined_wCOV/'],['Brain_LIabs_Rest1xTask_LOAD_',num2str(i),'x',num2str(j)],Brain_LD1_wCOV(length(nid)+1:2*length(nid),i),Brain_LD2_wCOV(length(nid)+1:2*length(nid),j));
%            
%         PlotCorr([statspath,'/Combined_wCOV/'],['Brain_HomoxLIabs_Rest1_LOAD_',num2str(i)],Brain_LD1_wCOV(1:length(nid),i),Brain_LD1_wCOV(length(nid)+1:2*length(nid),i));
%         PlotCorr([statspath,'/Combined_wCOV/'],['Brain_HomoxLIabs_Task_LOAD_',num2str(j)],Brain_LD2_wCOV(1:length(nid),j),Brain_LD2_wCOV(length(nid)+1:2*length(nid),j));
%    
%         homo_comb_ld1_wCOV=zeros(192,1);
%         homo_comb_ld2_wCOV=zeros(192,1);
%         homo_comb_ld1_wCOV(nid)=Brain_LD1_wCOV(1:length(nid),i);
%         homo_comb_ld2_wCOV(nid)=Brain_LD2_wCOV(1:length(nid),j);
%         
%         liabs_comb_ld1_wCOV=zeros(192,1);
%         liabs_comb_ld2_wCOV=zeros(192,1);
%         liabs_comb_ld1_wCOV(nid)=Brain_LD1_wCOV(length(nid)+1:2*length(nid),i);
%         liabs_comb_ld2_wCOV(nid)=Brain_LD2_wCOV(length(nid)+1:2*length(nid),j);
%         
%         SaveAsAtlasMZ3_Plot(homo_comb_ld1_wCOV,[statspath,'/Combined_wCOV/'],['Comb_Homo_Rest1_LOAD_' num2str(i)],[-0.15,0.15],shp);
%         SaveAsAtlasMZ3_Plot(homo_comb_ld2_wCOV,[statspath,'/Combined_wCOV/'],['Comb_Homo_Task_LOAD_' num2str(j)],[-0.15,0.15],shp);
%         SaveAsAtlasMZ3_Plot(liabs_comb_ld1_wCOV,[statspath,'/Combined_wCOV/'],['Comb_LIabs_Rest1_LOAD_' num2str(i)],[-0.15,0.15],shp);
%         SaveAsAtlasMZ3_Plot(liabs_comb_ld2_wCOV,[statspath,'/Combined_wCOV/'],['Comb_LIabs_Task_LOAD_' num2str(j)],[-0.15,0.15],shp);
%         
%     end
% end
% 
