...
system(['mkdir -p /data/stalxy/ArticleJResults/CAMCAN_Gretna/Results/RelatedinState_' nm '/All/']);
...
% [ALL_HxI_Rmap,ALL_HxI_Rsub]=gencorrmapCAM(FCS_Rstate,af,finalid,term(Aage)+term(Agender),filecorrpath{1},['All_' nm],[Aage,Agen],[-0.5,0.5])
[Y_HxI_Rmap,Y_HxI_Rsub]=gencorrmapCAM(FCS_Rstate,af,Yid,term(Yage)+term(Ygender),filecorrpath{4},['Youth_' nm],[Yage,Ygen],[-0.5,0.5])
[M_HxI_Rmap,M_HxI_Rsub]=gencorrmapCAM(FCS_Rstate,af,Mid,term(Mage)+term(Mgender),filecorrpath{2},['Middle_' nm],[Mage,Mgen],[-0.5,0.5])
[O_HxI_Rmap,O_HxI_Rsub]=gencorrmapCAM(FCS_Rstate,af,Oid,term(Oage)+term(Ogender),filecorrpath{3},['Old_' nm],[Oage,Ogen],[-0.5,0.5])
...
if sum(age_mediation)>0
    ...
    NiiProj2Surf(['/data/stalxy/ArticleJResults/CAMCAN_Gretna/Results/','/',['AgeMediation_' nm '_map'],'.nii'],'inf',projtype,'hemi',[0,1]);
end
